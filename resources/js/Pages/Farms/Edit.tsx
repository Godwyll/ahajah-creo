...
export default function Edit({ farm, onClose }: PageProps<{ farm: farm }> & { onClose: () => void }) {
    ...
    const [imagePreview, setImagePreview] = useState<string | null>(
        farm.image ? `/storage/${farm.image}` : null
    );
    ...
    function handleSubmit(e: React.FormEvent) {
        e.preventDefault();
        post(route('farms.update', { farm: farm.id }), {
            forceFormData: true,
            onSuccess: () => {
                onClose();
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6 p-6">
            <h2 className="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight mb-4">Edit farm</h2>
